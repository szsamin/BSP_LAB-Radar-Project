...
clearvars; 
saveData = []; timeData = []; global clear1; global toggle; Duration = 6; TOC = 0;
% [samplingRate] = samplingRatePrompt; 
newState = []; states = 0; newSteps = [];
...
stepTrack = uicontrol('Style','text',...
    'Position',[20 500 80 80],...
    'String','Value',...
    'FontSize',40);
    ...
    function toggle1(source,~)
        ...
        while(toggle==1 && isempty(toggle) ~= 1)
            ...
            while(isrunning(ai))
                ...
                set(stepTrack,'String',states);
                drawnow;
                
                
                if(clear1 == 1)
                    DAQclear(ai);
                    DataSavePrompt(timeData,saveData,samplingRate,states);
                    guiClear;
                    break;
                end
            end
